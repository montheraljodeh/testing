...
@Component({
  selector: 'app-digital-voucher-categories',
  templateUrl: './digital-voucher-categories.component.html',
  styleUrls: ['./digital-voucher-categories.component.scss'] 
})
export class DigitalVoucherCategoriesComponent implements OnInit {
  public rows?: any
  ...
  constructor(private digitalVoucherCategoriesService: DigitalVoucherCategoriesService, private cs: CommonService) {
  ...
  onEdit(event: any) {
    ...
    this.digitalVoucherCategoriesService.Update(event).subscribe(response => {
      if (response) {
        this.getByCriteria();
  ...
  getByCriteria() {
    this.digitalVoucherCategoriesService.GetALL(this.searchModel).subscribe(response => {
      this.cs.showOrHideSpinner(true);
      if (response) {
