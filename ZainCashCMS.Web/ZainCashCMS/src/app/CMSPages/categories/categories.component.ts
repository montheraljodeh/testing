...
export class CategoriesComponent implements OnInit {
  ...
  public coloumnName: CustomHeader[] = [
    ...
    {key:'mainImage',alias:'Image',hidden:true},
  ...
  constructor(private categoriesService: CategoriesService, private cs: CommonService ) {
  ...
  onEdit(event: any) {
    ...
    this.categoriesService.Update(event).subscribe(response => {
      if (response) {
        this.getByCriteria();
  ...
  getByCriteria() {
    ...
    this.categoriesService.GetALL(this.searchModel).subscribe(response => {
      this.cs.showOrHideSpinner(true);
      if (response) {
